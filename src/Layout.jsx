...
function LayoutContent({ children, currentPageName: _currentPageName }) {
  ...
  return (
    <>
      ...
      {/* Floating feedback widget — visible on every page inside the
          authenticated layout. Hidden on small auth/onboarding routes
          via the print:hidden class isn't enough but the widget gates
          itself on user existence in the input flow. */}
      {user && <FeedbackWidget />}
    </>
    );
          }
